...
import { QuickWaterTestForm } from "@/components/tanks/QuickWaterTestForm";
...
export default async function QuickWaterTestPage({ params }: RouteParams) {
          ...
          {result.kind === "ok" ? (
            <>
              <header className="space-y-2">
                <p className="text-sm font-medium text-aqua-deep">
                  Schnellerfassung
                </p>
                ...
                <p className="max-w-prose text-sm text-aqua-deep/75 md:text-base">
                  Messwerte direkt speichern, ohne Symptome oder sofortige Analyse.
                </p>
              </header>
              <QuickWaterTestForm tankId={result.tank.id} tankName={result.tank.name} />
            </>
